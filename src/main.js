...
function nextMatchday(value) { const date = new Date(`${value}T12:00:00`); while (![2, 5].includes(date.getDay())) date.setDate(date.getDate() + 1); return date }
...
function fixtures(chosen, legs, startDate) {
  ...
  const all = legs === 2 ? [...rounds, ...rounds.map(games => games.map(({ home, away }) => ({ home: away, away: home })))] : rounds; const date = nextMatchday(startDate)
  return all.flatMap((games, round) => { if (round) date.setDate(date.getDate() + (date.getDay() === 2 ? 3 : 4)); return games.map((game, index) => ({ ...game, id: `${round}-${index}`, round: round + 1, date: dateValue(date), homeScore: '', awayScore: '' })) })
}
...
function dashboard(autoScroll = false) {
  ...
  app.innerHTML = `<main class="league"><header class="league-head"><button class="text-button" data-leagues>כל הליגות</button><div><p class="eyebrow">עונת משחקים</p><h1>${league.name}</h1></div><button class="text-button" data-signout>התנתקות</button></header><section class="summary"><div><span>נבחרות</span><strong>${league.teams.length}</strong></div><div><span>משחקים שהוזנו</span><strong>${completed} <small>/ ${league.fixtures.length}</small></strong></div><div><span>ימי משחק</span><strong>ג׳ / ו׳</strong></div></section><div class="layout"><section><div class="heading"><div><p class="eyebrow">לוח המשחקים</p><h2>מתי משחקים?</h2></div></div>${dates.map(date => { const future = date > dateValue(); return `<details class="matchday ${date === closestDate ? 'closest' : ''} ${future ? 'future' : ''}" ${date === closestDate ? 'open' : ''}><summary><span>${dateText(date)}</span><small>מחזור ${groups[date][0].round}${future ? ' · ייפתח ביום המשחק' : ''}</small></summary><div class="games">${groups[date].map(game).join('')}</div></details>` }).join('')}</section><aside><div class="heading"><div><p class="eyebrow">טבלת הליגה</p><h2>מי מובילה?</h2></div><small>נקודות · הפרש · שערים</small></div><div class="table"><table><thead><tr><th>#</th><th>נבחרת</th><th>מש'</th><th>נ'</th><th>ת'</th><th>ה'</th><th>שע'</th><th>הפרש</th><th>נק'</th></tr></thead><tbody>${table().map((team,i) => `<tr><td>${i + 1}</td><td>${team.flag} ${team.name}</td><td>${team.played}</td><td>${team.wins}</td><td>${team.draws}</td><td>${team.losses}</td><td>${team.gf}:${team.ga}</td><td>${team.gf - team.ga > 0 ? '+' : ''}${team.gf - team.ga}</td><td><b>${team.points}</b></td></tr>`).join('')}</tbody></table></div></aside></div></main>`
...
async function createLeague(form) {
  ...
  const data = new FormData(form); const legs = +data.get('legs'); const item = { name: data.get('name').trim() || 'הליגה שלי', teams: selected, gamesPerWeek: 2, fixtures: fixtures(selected, legs, data.get('startDate')) }
